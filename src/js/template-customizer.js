...
class TemplateCustomizer {
  ...
  _setup(_container = document) {
    ...
    const resetBtnCb = () => {
      ...
      document.cookie = 'django_language=fa;path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;'
      document.cookie = 'django_text_direction=rtl;path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;'
    ...
    if (!this._hasControls('rtl headerType contentLayout layoutCollapsed layoutNavbarOptions', true)) {
      ...
      if (!this._hasControls('rtl') || !rtlSupport) {
        ...
        const rtlCb = e => {
          this._loadingState(true)
          // For demo purpose, we will use EN as LTR and Fa as RTL Language
          if (e.target.value === 'rtl') {
            this._setSetting('Lang', 'fa')
            document.cookie = 'django_language=fa;path=/'
...
TemplateCustomizer.LANGUAGES = {
  ...
  fr: {
    ...
    misc_header: 'DIVERS',
    rtl_label: 'Direction RTL'
  },
  ar: {
    Panel_header: 'أداة تخصيص الصفحة',
    Panel_sub_header: "التخصيص والمعاينة في الوقت الفعلي",
    themeing_header: 'THEMING',
    theme_header: "الموضوع",
    theme_label: "الموضوعات",
    style_label: 'النمط (الوضع)',
    style_switch_light: 'الضوء',
    style_switch_dark: 'مظلم',
    layout_header: "تخطيط",
    Layout_label: "التخطيط (القائمة)",
    Layout_static: "ثابت",
    Layout_offcanvas: 'Offcanvas',
    Layout_fixed: "ثابت",
    Layout_fixed_offcanvas: 'تم إصلاح offcanvas',
    Layout_flipped_label: "القائمة مقلوبة",
  ...
  de: {
    panel_header: 'Die Liste wird geöffnet',
    panel_sub_header: 'Anpassen und Vorschau in Echtzeit',
    theming_header: 'THEMING',
    theme_header: 'THEME',
    theme_label: 'Themen',
    style_label: 'Stil (Modus)',
    style_switch_light: 'Licht',
    style_switch_dark: 'Dunkel',
    layout_header: 'LAYOUT',
    layout_label: 'Layout (Menü)',
    layout_static: 'Statisch',
    layout_offcanvas: 'Offcanvas',
    layout_fixed: 'Behoben',
    layout_fixed_offcanvas: 'Offcanvas behoben',
    layout_flipped_label: 'Menü umgedreht',
    layout_dd_open_label: 'Dropdown beim Hover',
    layout_navbar_label: 'Navigationsleiste korrigiert',
    layout_footer_label: 'Fußzeile korrigiert',
    misc_header: 'MISC',
    rtl_label: 'RTL-Richtung'
  },
  fa: {
